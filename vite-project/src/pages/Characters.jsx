...
const Characters = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // se envia 'image' porque asi esta en tu tabla de sql
    ...
    <div className="relative min-h-screen w-full bg-black text-white font-mono">
      ...
      <div className="relative z-10 p-8 pt-24 flex flex-col items-center">
        ...
        <div className="flex gap-4 mb-10 z-20">
          <button onClick={() => { setShowForm(!showForm); setShowList(false); }} className={`px-5 py-2 border-2 uppercase text-xs font-bold ${showForm ? 'bg-red-700 text-white border-red-700' : 'bg-white text-black border-white'}`}> [ add_new_entity ] </button>
          <button onClick={() => { setShowList(!showList); setShowForm(false); }} className={`px-5 py-2 border-2 uppercase text-xs font-bold ${showList ? 'bg-red-700 text-white border-red-700' : 'bg-white text-black border-white'}`}> [ view_database ] </button>
        </div>
        {showForm && (
          <section className="w-full max-w-xl bg-white p-8 mb-12 border-4 border-zinc-900">
        ...
        {showList && (
          <div className="w-full max-w-5xl mx-auto mb-20">
            {characters.length === 0 ? <p className="text-center py-20 border border-dashed border-zinc-800 text-zinc-500">-- no_entities_registered --</p> : 
              characters.map((char) => (
                <div key={char.char_id} className="bg-black/80 border border-white p-5 flex gap-5 mb-4 backdrop-blur-sm">
                  <div className="w-32 h-32 bg-zinc-900 border border-zinc-800 flex-shrink-0 overflow-hidden">
                    {/* se usa char.image segun tu sql */}
