...
const Characters = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // He corregido los nombres de las propiedades para asegurar compatibilidad
  ...
  const inputStyle = "bg-white border-2 border-zinc-300 w-full p-3 text-black focus:border-black outline-none uppercase text-sm font-bold placeholder:text-zinc-400";
    ...
    <div className="relative min-h-screen w-full bg-black text-white font-mono overflow-x-hidden">
      ...
      <div className="relative z-10 p-8 pt-24 flex flex-col items-center">
        ...
        <div className="flex gap-4 mb-10 z-20">
          <button onClick={() => { setShowForm(!showForm); setShowList(false); }} className={`px-5 py-2 border-2 uppercase text-xs tracking-widest font-bold transition-all ${showForm ? 'bg-red-700 border-red-700 text-white' : 'bg-white border-white text-black'}`}>
            {showForm ? '[ Cancel_Entry ]' : '[ Add_New_Entity ]'}
          </button>
          
          <button onClick={() => { setShowList(!showList); setShowForm(false); }} className={`px-5 py-2 border-2 uppercase text-xs tracking-widest font-bold transition-all ${showList ? 'bg-red-700 border-red-700 text-white' : 'bg-white border-white text-black'}`}>
              ...
              <div className="grid grid-cols-1 gap-6">
                {characters.map((char) => (
                  <div key={char.id} className="bg-black/80 border border-white p-5 flex flex-col md:flex-row gap-5 backdrop-blur-sm">
                    <div className="w-full md:w-32 h-32 bg-zinc-900 border border-zinc-700 flex-shrink-0 flex items-center justify-center font-bold text-zinc-700">NO_IMG</div>
