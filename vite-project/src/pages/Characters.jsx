...
const Characters = () => {
  ...
  return (
    <div className="relative min-h-screen w-full bg-black text-white font-mono">
      <img src={charactersBg} className="fixed inset-0 w-full h-full object-cover z-0" alt="background" />
      ...
      <div className="relative z-10 p-8 pt-24 flex flex-col items-center">
        ...
        <div className="flex gap-4 mb-10 z-20">
          <button onClick={() => { setShowForm(!showForm); setShowList(false); }} className={`px-5 py-2 border-2 uppercase text-xs font-bold transition-all ${showForm ? 'bg-red-700 text-white border-red-700' : 'bg-white text-black border-white'}`}> [ add_new_entity ] </button>
          <button onClick={() => { setShowList(!showList); setShowForm(false); }} className={`px-5 py-2 border-2 uppercase text-xs font-bold transition-all ${showList ? 'bg-red-700 text-white border-red-700' : 'bg-white text-black border-white'}`}> [ view_database ] </button>
        </div>
        ...
        {showList && (
          <div className="w-full max-w-5xl mx-auto mb-20 bg-black/40 backdrop-blur-sm p-4 border border-white/20">
            {characters.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {characters.map((char) => (
                  <div key={char.char_id} className="bg-black/90 border border-white/30 p-3 flex items-center gap-6 hover:border-white transition-all">
                    <img src={char.image} className="w-20 h-20 object-cover border border-zinc-700" alt={char.name} />
                    <div className="flex-1">
                      <h2 className="text-lg uppercase font-bold leading-none">{char.name}</h2>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="px-3 py-1 border border-red-900 text-[10px] text-red-500 uppercase hover:bg-red-900 hover:text-white transition-all">delete</button>
                    </div>
                  </div>
                ))}
              </div>
